...
落地约定〔评审 P2-1/P2-2 + §2.2〕：① 请求体一律经 `services.validation.json_body()`，
非对象 / 缺 Content-Type 的畸形 / 空体降级为 `{}`；② 合法 token 但用户已删（current_user() is None）
...
def update_profile():
    ...
    if user is None:
        return jsonify({"error": "unauthorized"}), 401
    data = json_body()  # 【§2.2】非对象体 → {}（此前 "display_name" in data 触 500）
...
def _apply_profile(user, data) -> str | None:
    ...
    if "display_name" in data:
        name = want_str(data, "display_name")  # 非串 → 400（此前 .strip() 500）
...
def change_password():
    ...
    if user is None:
        return jsonify({"error": "unauthorized"}), 401
    data = json_body()
    # 非串密码 → 400（此前 check_password 触 500）；密码不 strip（保留原字符）。
    current_password = want_str(data, "current_password", strip=False)
    new_password = want_str(data, "new_password", strip=False)
...
def update_notification_preferences():
    ...
    if user is None:
        return jsonify({"error": "unauthorized"}), 401
    data = json_body()  # 【§2.2】非对象体 → {}（此前 data.get 触 500）
